...
namespace OsmToolkit.Tests.Mcp
{
    ...
    /// would, over its real stdio transport - unlike <see cref="SearchByTagsInAreaHandlerTests"/>, which calls
    ...
    /// <c>OverpassOsmDataSourceManualTests</c>. Run it manually with: dotnet test --filter "TestCategory=ManualIntegration"
    ...
    public class SearchByTagsInAreaManualTests
    {
        [TestMethod]
        public async Task SearchByTagsInArea_OverRealStdioServer_AppearsInToolListAndReturnsCafesInGamlebyen()
        {
            // Arrange
            ...
            var result = await client.CallToolAsync(
                ...
                new Dictionary<string, object?>
                {
                    ...
                    ["tags"] = new Dictionary<string, object?> { ["amenity"] = "cafe" },
                },
                cancellationToken: timeout.Token);

            // Assert
            TestContext.WriteLine($"Tools advertised by the running server: {string.Join(", ", tools.Select(t => t.Name))}");
            Assert.IsTrue(tools.Any(t => t.Name == "search_by_tags_in_area"), "search_by_tags_in_area was not registered with the running MCP server.");
            ...
            Assert.IsFalse(string.IsNullOrWhiteSpace(text), "Expected a non-empty result for cafes in Gamlebyen.");
        }
