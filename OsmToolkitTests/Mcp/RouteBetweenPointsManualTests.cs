...
namespace OsmToolkit.Tests.Mcp
{
    ...
    /// would, over its real stdio transport - unlike <see cref="RouteBetweenPointsHandlerTests"/>, which calls
    ...
    /// <see cref="SearchByTagsInAreaManualTests"/> and <see cref="FindNearPointManualTests"/>.
    ...
    public class RouteBetweenPointsManualTests
    {
        [TestMethod]
        public async Task RouteBetweenPoints_OverRealStdioServer_AppearsInToolListAndReturnsARoute()
        {
            // Arrange
            ...
            var result = await client.CallToolAsync(
                ...
                {
                    // Two nearby, well-known points within Gamlebyen, Fredrikstad: small enough that the
                    // public Overpass instance won't hit its own server-side query timeout, real enough
                    // that a foot route between them should exist in OSM data.
                    ["origin"] = "Gamlebyen, Fredrikstad",
                    ["destination"] = "Fredrikstad, Norway",
                    ["travelMode"] = "foot",
                    ["avoidMotorway"] = false,
                },
            ...
            Assert.IsTrue(tools.Any(t => t.Name == "route_between_points"), "route_between_points was not registered with the running MCP server.");
            ...
            Assert.IsFalse(string.IsNullOrWhiteSpace(text), "Expected a non-empty result for a route between Gamlebyen and Fredrikstad.");
        }
